...
policy. The proposed black-box search modifies the target, timing, visibility,
duration, and intensity of fixed multi-stage campaigns. An independent validator enforces
network reachability, privilege, and temporal constraints, and closed-loop
replay checks physical execution. Every candidate is re-executed in a
three-stage industrial process. The returned explanation is the lowest-cost
qualifying campaign found within the declared search budget. The evaluation compares random,
...
CIPHER-A uses LLM-generated counterfactual consequence scenarios to validate
response-plan actions and updates an attacker model after execution
~\cite{kizhakel2026cipher}. Its object is the predicted consequence of a
proposed defensive action in enterprise incident response. The object here is
a minimal executable change to the attack campaign that flips a frozen RL
responder, with the changed physical consequence reproduced in a closed ICPS
loop.
...
\begin{equation}
\begin{aligned}
C(\Delta)=\sum_j\bigl(&w_v|\Delta v_j|+w_t|\Delta t_j|/T\\
 &+w_d|\Delta d_j|/T+w_m|\Delta m_j|
 +w_a\mathbb{I}[\Delta a_j]\bigr),
\end{aligned}
\end{equation}
where $v,t,d,m,a$ denote visibility, time, duration, magnitude, and target
asset.
...
cost. Simulation outcomes for duplicate campaign signatures are cached. When
the same campaign is reached through several edit paths, the lowest-cost path
is retained. Search terminates at a fixed depth and evaluation budget; the
least-cost satisfying candidate found inside that bounded search is revalidated
...
without using its recorded data. Measurement replay changes only its targeted
delivered sensor while retaining the hidden process state. An inlet-valve
override changes inflow, while a transfer-pump override changes inter-stage
flow.
...
counterfactuals, cyber-valid search whose frontier and final selection do not
use physical outcomes, and the complete cyber-physical method. Target validity is the fraction that selects a
...
wall-clock time, and explanation transfer under the perturbed execution model.
...
using medians, interquartile ranges, and paired bootstrap intervals. Because the
four scenarios evaluated under one learned policy are correlated, paired
validity differences are first averaged within each policy and the bootstrap
resamples policy seeds.
